...
async function initTasks() {
  ...
  try {
    ...
    allTasks = (data.tasks || []).filter(t => t.assignedUser === currentUser.name);
...
function saveTasks() {
    ...
    if (stored) {
        ...
        // Replace current user's tasks in the main database
        const otherUserTasks = data.tasks.filter(t => t.assignedUser !== currentUser.name);
        data.tasks = [...otherUserTasks, ...allTasks];
        localStorage.setItem('lexflow_mock_data', JSON.stringify(data));
    }
}
