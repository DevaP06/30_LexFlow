...
async function initCases() {
  const currentUser = JSON.parse(sessionStorage.getItem('lexflow_current_user')) || { name: 'Sarah Mitchell', avatar: 'SM', id: 'ADM001' };
  ...
  try {
    ...
    // Only show cases assigned to Sarah Mitchell (ADM001)
    allCases = data.cases.filter(c => c.assignedAdvocateId === 'ADM001');
    allTasks = data.tasks || [];
    
    // Default filter
    filteredCases = [...allCases];

    // Compute pending tasks badge for Sarah's cases
    const pendingTasks = allTasks.filter(t => t.status === 'Pending' && t.assignedUser === 'Sarah Mitchell');
...
function applyFilters() {
    ...
    if (currentTab === 'all') {
        ...
        let temp = [...allTasks].filter(t => t.assignedUser === 'Sarah Mitchell');
  ...
  return `
    <div class="case-card page-item" data-title="${c.title.toLowerCase()}" data-cnr="${c.cnr.toLowerCase()}" onclick="window.location.href='case-details.html?cnr=${c.cnr}'" style="display:flex; justify-content:space-between; align-items:center; gap: 24px;">
      ...
      <div class="case-info-left" style="display:flex; align-items:center; gap: 20px; flex: 1; min-width: 0;">
          <!-- Using the original case-icon from client side for animation consistency -->
          <div class="case-icon">
