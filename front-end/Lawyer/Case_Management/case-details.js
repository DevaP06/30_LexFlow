...
async function initCaseDetails() {
  ...
  if (currentUser) {
    ...
    document.getElementById('sidebarName').textContent = 'Sarah Mitchell';
    document.getElementById('sidebarAvatar').textContent = 'SM';
  }
  ...
  try {
    ...
    if (stored) {
      ...
      const response = await fetch('../../shared/data/mock-data.json');
...
function renderTeam() {
    let advName = "Sarah Mitchell";
    let advRole = "Lead Attorney";
    if(currentCase.assignedAdvocateId === "ADM002") advName = "Vikram Nair";
    if(currentCase.assignedAdvocateId === "ADM003") advName = "Ananya Iyer";

    teamContainer.innerHTML = `
        <div style="display: flex; gap: 12px; align-items: center;">
            <div style="width: 32px; height: 32px; border-radius: 50%; background: #eef2ff; color: #3b5bdb; display: flex; align-items:center; justify-content:center; font-size: 11px; font-weight:700;">${advName.substring(0,2).toUpperCase()}</div>
            <div style="display:flex; flex-direction:column;">
                <span style="font-size:13px; font-weight:700; color:#1a1a2e;">${advName}</span>
                <span style="font-size:11px; color:#6b7280;">${advRole}</span>
            </div>
        </div>
        <div style="display: flex; gap: 12px; align-items: center;">
...
function renderClientInfo() {
    const parts = currentCase.title.split('vs.');
    let c1 = parts[0] ? parts[0].trim() : 'Unknown';
    let c2 = parts[1] ? parts[1].trim() : 'Unknown';
    if(c1 !== 'Property Dispute') {
        clientContact.textContent = c1;
    }
    opposingParty.textContent = c2.replace('Ltd.', '').trim();
}
...
function renderPendingTasks() {
    ...
    pendingTasksContainer.innerHTML = pending.slice(0, 3).map(t => {
        return `
...
// Wiring Up Export & Edit Details from top actions
setTimeout(() => {
    const editBtn = document.querySelector('.topbar ~ .content .card .filter-btn:nth-child(1)');
    if (editBtn) editBtn.addEventListener('click', openEditCaseModal);
    
    const expBtn = document.querySelector('.topbar ~ .content .card .filter-btn:nth-child(2)');
